...
$_SESSION['cform-rand'] = isset($_SESSION['cform-rand']) ? $_SESSION['cform-rand'] : rand(100, 999);

// The shortcode
function cform_shortcode($vscf_atts) {
	$vscf_atts = shortcode_atts( array( 
		...
		"message_success" => __('Thank you! You will receive a response as soon as possible.', 'simple-contact-form'),
		"hide_subject" => ''
	), $vscf_atts);
	...
	$email_form = '<form class="vscf" id="vscf" method="post">
		...
		<p><label for="cform_captcha">'.sprintf(esc_attr($vscf_atts['label_captcha']), $_SESSION['cform-rand']).': <span class="'.(isset($error_class['form_captcha']) ? "error" : "hide").'" >'.esc_attr($vscf_atts['error_captcha']).'</span></label></p>
	...
	if(isset($sent) && $sent == true) {
		unset($_SESSION['cform-rand']);
		return $info;
	} else {
		return $email_form;
	}
} 
add_shortcode('contact', 'cform_shortcode');
